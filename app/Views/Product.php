...
class Product extends BeerPlanet implements \Interfaces\Presentable {
    ...
    /**
     * This is a mandatory method for all view pages.
    ...
    public function addView($message = false) {
        ...
        $typeSubTypes = $this->_service->getTypeSubTypes(
                array_shift($types)->getId());
        $this->_smarty->assign('typeSubtypes', $typeSubTypes);
        $this->_smarty->assign('subtypes', $subTypes);
        $this->_smarty->assign('ratings', 
                $this->_service->getAveragesForProducts($entities));
    ...
    /**
     * Method shows single element from database.
     * When object is requested, it
    ...
    public function detailsView($id, $message = false) {
        if ($id > 0) {
            ...
            if (! $product) {
                $this->setMessage(
                        new \Entities\Message(\Enum\MessageType::Error, 
                ...
                $this->_openGraph->setTitle(
                        $product->getManufactor() . ' \'' . $product->getName() .
                                 '\' ' . '(' . $average . '/10)');
                $this->_openGraph->setDescription(
                        $this->_translation['type'] . ': ' . $array['typeId'] .
                                 ', ' . $this->_translation['subtype'] . ': ' .
                                 $array['subTypeId'] . ', ' .
                                 $this->_translation['alc'] . ': ' .
                                 sprintf('%.1f %%', $product->getAlc()) . ', ' .
                                 $this->_translation['origin'] . ': ' .
                                 $product->getOrigin());
                $this->_openGraph->setUrl(
                        LIKEURL . '/product/details/' . $product->getId());
    ...
    public function listView($message = false) {
        ...
        $this->_entity->setTypeId($this->_drinkType);
        $this->loadFreshProductsToSmarty($this->_entity);
    ...
    /**
     * Method populates smarty variables with fresh product data
     * 
     * @param unknown $entity            
     */
    public function loadFreshProductsToSmarty($entity) {
        $entities = $this->_service->getAll($entity);
        $fresh = array_slice($entities, 0, 4);
        $imgnames = array();
        foreach ($fresh as $k => $itm) {
            $imgnames[$itm->getId()] = $this->getImgName($itm);
    ...
    public function addAction($post) {
        ...
        $filename = $this->_createName($post['manufactor'], $post['name']) . '.' . \Misc\Image::getFileExtension(
                $_FILES["picture"]["name"]);
        // upload image to server
        \Misc\Image::uploadImage($_FILES, $filename);
        // create thumbnail on the fly
        \Misc\Image::createCroppedThumbnail(APPPATH . 'uploads/', $filename, 
                self::IMG_X, self::IMG_Y, 'thumb');
    }
}