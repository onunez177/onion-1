<?php
/**
 * File for misc configurations. Add Your own config here also!
 */

/**
 * Conf variable whether to set names to utf8 or not. Set to true if non-latin
 * characters break when selecting from database!
 * @var boolean
 */
define('SETDBENCODING', true);

/**
 * Name of the image that is used as a default image when nothing is uploaded
 * Path to this image is found in paths.php (IMAGES)
 * @var var
 */
define('NOIMAGE', 'noimage.png');

/**
 * URL to page that users can like on Facebook
 * @var var
 */
define('LIKEURL', '127.0.0.1');